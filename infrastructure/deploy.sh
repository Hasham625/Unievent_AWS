...
#   * IAM role + instance profile (least-privilege S3 access)
#   * Encrypted, private S3 bucket for event posters
#   * Application Load Balancer in public subnets
#   * Auto Scaling Group of 2 EC2 instances (Amazon Linux 2023) in private subnets
#
# Usage:
...
UD_TEMPLATE="$SCRIPT_DIR/userdata.sh"
...
echo "==> NAT Gateway + private route table (waiting ~1 min for NAT to be ready)"
...
echo "==> Encrypted private S3 bucket"
...
python3 - "$UD_TEMPLATE" "$APP_PY" "$TM_KEY" "$BKT" > /tmp/userdata_rendered.sh <<'PY'
import sys
ud_path, app_path, tm_key, bucket = sys.argv[1:5]
with open(ud_path) as f: template = f.read()
with open(app_path) as f: app_py  = f.read()
rendered = (template
            .replace("__APP_PY_PLACEHOLDER__", app_py)
